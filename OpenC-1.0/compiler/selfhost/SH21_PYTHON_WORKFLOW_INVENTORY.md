...
| `scripts/source_completeness.py` and `source_inventory.py` | Source inventory and authored-source completeness | Implement deterministic OpenC source manifest audit | Implemented for 366 required files |
...
fixed-point closure, deterministic JSON reporting, the 1,323-record native
repository audit, the 16-check native PE/import/unwind/CRT audit, and the 42-check
native framed LSP audit. Native child execution
...
default five-minute timeout. It does not claim complete benchmark sampling or
ZIP ownership until those capabilities move into OpenC.
...
1. Add native benchmark sampling and performance gates.
2. Add directory enumeration/creation and deterministic ZIP emission for
   `openc release`.
