...
unsafe i32 cli_workflow_command() {
    ...
    if !cli_release_ensure_directory(path.directory(output_path)) {
        io.error("error: workflow output directory unavailable\n");
        return 1;
    }
    ...
    cli_workflow_command_named_argument(command, "--root=", root);
    cli_workflow_command_named_argument(
        command, "--artifacts=", com_winrt_artifacts
    );
    cli_workflow_command_named_argument(
        command, "--output=", com_winrt_audit_report
    );
    cli_workflow_execute(
        report, counters, "native_com_winrt_projection", command,
        "OpenC COM/WinRT audit: PASS (33/33)"
    );
    d_buffer_destroy(command);
