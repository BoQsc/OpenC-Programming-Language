...
| 2-3. Fused semantics and lowering | Isolated Phase B2 two-lane NO-GO; redesign open | The scalar-sweep fusion was correct but speed-flat/regressive. [Function semantic/IR Phase B2](https://github.com/BoQsc/OpenC-Programming-Language/blob/89dd8d4/OpenC-1.0/compiler/selfhost/SH27_FUNCTION_SEMANTIC_IR_B2_EVIDENCE.md) achieved exact 8/8 large and 4/4 control synthetic-source coverage with deleted assignment/binary sweeps. Guarded 11-pair/null whole-wall improved large by 16 ms but regressed control by 10 ms; self-build remained 0/223 fast sources. Profile migrated pointer-order/IR first visits and broaden self-build eligibility before any promotion; no eager extra pass or duplicate generic-record arena. |
...
| 6. Incremental objects | Isolated interface, COFF-identity, and post-lowering module-object prerequisites passed; actual object reuse not implemented | The [interface projection](https://github.com/BoQsc/OpenC-Programming-Language/blob/c0b2343/SH27_INTERFACE_FINGERPRINT_EVIDENCE.md) passed 11 checks; [stable COFF names](https://github.com/BoQsc/OpenC-Programming-Language/blob/f68c65f/SH27_STABLE_COFF_IDENTITY_EVIDENCE.md) passed ten. The [opt-in module COFF set](https://github.com/BoQsc/OpenC-Programming-Language/blob/bd0a497/SH27_MODULE_COFF_SET_EVIDENCE.md) linked and ran two cross-module objects with stable extern names, but still partitions a whole-project lowered stream. Its Stage 2 exceeded strict 64 MiB working set. `SH27_INCREMENTAL_OBJECT_REUSE_ARCHITECTURE.md` still requires independent module acceptance/lowering, shared-data handling, native multi-object relink, and an atomic content-validated cache. Demonstrate real object hits and correct implementation/API invalidation, not merely a warm timer. |
