...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="oc-cache-") as temporary:
        ...
        canonical = "openc-module-cache-v1:windows-x86_64-llp64:stable-coff:serial\n" + project_text
        ...
        for source in initial_sources.values():
            ...
            canonical += hashlib.sha256(projection.encode()).hexdigest() + "\n"
        global_hash = hashlib.sha256(canonical.encode()).hexdigest()
        for module, source in initial_sources.items():
            ...
            key = hashlib.sha256((global_hash + content_hash).encode()).hexdigest()
        ...
        # identity. Conservative interface invalidation must rebuild all three.
        ...
        print("automatic module cache: hits, body/API invalidation, corruption/size/key/identity recovery, concurrent publication, unavailable storage and full diagnostics passed")
