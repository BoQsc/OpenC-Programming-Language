...
20. **SH-27 post-release integrity and production performance — active**
    ...
    - the first bounded generated corpus, one-source-edit lane, parallel batch,
      ...
      implemented; clean Windows runs through 34810131926 rebuild current
    ...
    - semantic gating eliminates the 13.138-second pointer-arithmetic scan on
      ...
      seconds; indexed expression/block/call/control/return traversal then
      reduces the large median to 4.039 seconds; semantic source/project gates
      then reduce empty flow groups to zero and the median to 2.797 seconds,
      and the stateless-source flow fast path reduces it to 2.223 seconds,
      before bounded prefix-type lookup reduces it to 1.808 seconds, 93.0%
      below the original baseline. Exact parsed-source reuse then removes
      lowering's reparse and reduces the median to 1.550 seconds, 94.0% below
      the original baseline. Moving cache creation into declaration collection
      then removes resolution parsing and reduces the median to 0.931 seconds,
      96.4% below the original baseline. Small and many-file parity passes;
      flow validation then reuses the same cache and records zero parse time.
      Single-pass feature discovery then reduces the clean large median to
      0.805 seconds, 96.9% below the original baseline. A 1.235x Clang result
      was observed once but was not sustained by the next 1.342x run;
      candidate-gated expression passes and already-ordered IR emission then
      produce two consecutive passing Clang ratios of 1.218x and 1.213x.
      IR-bounded call-free constant pools reduce isolated native-emission work;
      the latest shared-runner Clang ratio moves to 1.302x, so Clang parity is
      still intermittent. Indexed duplicate-function validation then removes a
      quadratic all-functions comparison, reduces clean acceptance to 187–204
      milliseconds, and restores a 1.120x Clang pass. Checked bulk native
      ...
      passes.
      IR-bounded code and relocation scratch for call-free, scope-free native
      functions then reduces the paired local large median from 1.500 to 1.453
      seconds with byte-identical output. Clean run 34809511246 records 1.091
      seconds: 2.118x MSVC, 1.117x Clang, and 3.209x DMD64; every small and
      many-file gate still passes.
      Separating the ordinary four-edge relocation bound from six-edge cast
